...
from .views import create_or_get_viatge, get_or_edit_or_delete_viatge, get_viatges_participant_or_join, eject_viatge
...
class ViatgeTestCase(TestCase):
    ...
    def test_join_viatge(self):
        ...
        request = self.factory.post(f'/usuaris/{email2}/viatges/share', data=request_data)

        response = get_viatges_participant_or_join(request, email2)
    ...
    def test_get_participants_viatge(self):
        email = "polsalvador1@gmail.com"
        usuari = Usuari.objects.create(email=email)
        ...
        get_viatges_participant_or_join(request, email2)

        request = self.factory.get(f'/usuaris/{email2}/viatges/share')

        response = get_viatges_participant_or_join(request, email2)

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(viatge.participants.filter(email=email2).exists())
    

    def test_eject_viatge(self):
        email = "polsalvador1@gmail.com"
        usuari = Usuari.objects.create(email=email)
        ...
        request = self.factory.delete(f'/usuaris/{email2}/viatges/{viatge.id}/share')

        response = eject_viatge(request, email2, viatge.id)

        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        viatge_refreshed = Viatge.objects.get(id=viatge.id)
        participants = viatge_refreshed.participants.filter(email=email2)
        self.assertFalse(participants.exists())
